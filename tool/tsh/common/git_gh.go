...
func (h *gitHTTPProxyHandler) loadKMSToken() []byte {
	proxyHost, _, _ := net.SplitHostPort(h.tc.WebProxyAddr)
	data, err := client.LoadUserSecret(h.homePath, proxyHost, h.tc.SiteName, h.gitServerName)
	if err != nil || data == nil {
		return nil
	}

	// Backward compat: if only the old field is populated, use it directly.
	if len(data.DoubleEncryptedToken) == 0 {
		return data.AuthEncryptedToken
	}
	...
	kmsBlob, err := helper.decrypt(context.Background(), data.DoubleEncryptedToken)
	if err != nil {
		logger.WarnContext(context.Background(), "Failed to decrypt cached token, removing stale cache", "error", err)
		proxyHost, _, _ := net.SplitHostPort(h.tc.WebProxyAddr)
		client.DeleteUserSecret(h.homePath, proxyHost, h.tc.SiteName, h.gitServerName)
		return nil
	}
	return kmsBlob
}
