...
- Blockers: unresolved guesses that prevent approval.
...
order only controls missing questions. Ask one stage at a time and stop for the
user's answer before advancing. For how to present a stage's questions, see
SKILL.md's Presentation section.

Create is a stage gate, not a checklist to batch together. On each turn, find
the earliest incomplete stage and ask only for that stage. SECURITY.md's general
...
If a response lists candidates (including expanded AWS IC assignments), previews
a selector, asks for confirmation, or asks for resource-specific principals or
identities, do not ask Stage 2 access type or Stage 3 framing in that same
response.
...
- Once labels, selector, or exact identifiers are settled, preview or confirm
  the selection per RESOURCE_KINDS.md.
- If that preview or confirmation asks the user to confirm, narrow, adjust, or
  accept risk for the selection, ask only that question; do not mention other
  missing fields yet. Stop and wait for the answer.
- Ask resource-specific principal or identity values required by
  RESOURCE_KINDS.md's Kind Map and Application Identities only after the
  selection is confirmed. If any are missing, ask only for those values and
  stop. Do not ask for access type, owners, title, or description in the same
  response.

Do not ask for access type, owners, title, or description in this stage unless
the user already gave them.
