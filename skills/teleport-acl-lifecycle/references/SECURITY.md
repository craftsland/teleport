...
- Resolve any access-list title to its identifier — update/delete targets, and
  ...
  `.metadata.name`, scope `.scope`,
  description `.spec.description`) to count case-insensitive substring matches
  against what the user said. Users rarely recall a title verbatim, so a title
  that merely contains the queried text is a candidate too, not just an
  ...
  disambiguates. A match found only in `.spec.description`, with no match in
  title, name, or scope, is weak evidence — it does not count as the "one
  match" that lets you continue silently; treat it like a multi-match case.
  One match on title, name, or scope: continue. More than one, or a
  description-only match: surface every candidate the filter returned,
  including any weak description-only one, with its title, identifier, scope,
  access type derived per PRESETS.md, and description; never auto-select, and
  stop until the user picks. Bundle title, identifier, access type, and
...
3. Check its exit status and JSON once. A non-zero exit or invalid JSON means
   `tctl` failed: relay stderr and stop.
...
5. A response that renders a menu, question, preview, draft, or approval is
   ...
   failure, not a rerun reason; relay it and stop. A wrapper that loses the path
   is not a reason to re-run: inspect the known paths first.
6. Remove only after a read failure; the user explicitly says they are done or
   cancels; a later user request starts a new unrelated workflow; or the final
   approved write completes. Rendering a menu, question, preview, draft, or
   approval is never an end. Remove only the exact directory created in step 1
   with `rm -rf -- "$capture_dir"`; never reconstruct, glob, or broaden the path.
