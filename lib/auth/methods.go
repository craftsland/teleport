...
func (a *Server) authenticateUser(
	...
	verifyLocks = func(p verifyMFADeviceLocksParams) error {
		...
		return a.verifyLocksForUserCerts(verifyLocksForUserCertsReq{
			checkerContext: p.CheckerContext,
...
func (a *Server) AuthenticateSSHUser(ctx context.Context, req authclient.AuthenticateSSHRequest) (*authclient.CLILoginResponse, error) {
	...
	if len(req.EncryptionPublicKey) > 0 {
		...
		if err := services.UpsertUserEncryptionKey(ctx, encKeySvc, user.GetName(), pb.EncryptionKeyEntry_builder{
			KeyId:     sessionID,
			PublicKey: req.EncryptionPublicKey,
			CreatedAt: timestamppb.New(a.clock.Now()),
			Expires:   timestamppb.New(certExpiry),
		}.Build()); err != nil {
			a.logger.WarnContext(ctx, "Failed to register encryption key", "error", err)
		}
	}
