...
func (fs *FSKeyStore) AddKeyRing(keyRing *KeyRing) error {
	...
	// Store ECIES encryption private key if present. Wipe any cached
	// user secrets since they were encrypted with the previous key.
	if keyRing.EncryptionPrivateKey != nil {
		...
		userSecretsDir := filepath.Join(filepath.Dir(fs.KeyDir), "user_secrets", keyRing.ProxyHost)
		os.RemoveAll(userSecretsDir)
	}
...
func (fs *FSKeyStore) DeleteKeyRing(idx KeyRingIndex) error {
	...
	// Remove cached user secrets (double-encrypted tokens).
	userSecretsDir := filepath.Join(filepath.Dir(fs.KeyDir), "user_secrets", idx.ProxyHost)
	if err := utils.RemoveAllSecure(userSecretsDir); err != nil && !errors.Is(err, iofs.ErrNotExist) {
		deleteErrs = append(deleteErrs, trace.Wrap(err))
	}
