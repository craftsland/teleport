...
func EventToGRPC(in types.Event) (*proto.Event, error) {
	...
	case types.Resource153UnwrapperT[*userexternalsecretsv1.UserEncryptionKeys]:
		out.Resource = &proto.Event_UserEncryptionKeys{
			UserEncryptionKeys: r.UnwrapT(),
		}
...
func EventFromGRPC(in *proto.Event) (*types.Event, error) {
	...
	if r := in.GetResourceHeader(); r != nil {
		...
		out.Resource = types.Resource153ToLegacy(r)
		return &out, nil
	} else if r := in.GetUserEncryptionKeys(); r != nil {
		out.Resource = types.Resource153ToLegacy(r)
		return &out, nil
